...
const SCORE_BG = ["", "#2c1010", "#2c1a08", "#2c2408", "#0a2c1a", "#081f12"];
...
export default function SoC13DealQA() {
  ...
  const getVerdict = (avg: string | null) => {
    ...
    if (n >= 4.5) return { label: "PROCEED - HIGH CONFIDENCE", color: "#27ae60" };
    if (n >= 3.8) return { label: "PROCEED WITH CONDITIONS", color: "#2ecc71" };
    if (n >= 3.0) return { label: "CONDITIONAL - FURTHER DD REQUIRED", color: "#f1c40f" };
    if (n >= 2.0) return { label: "HIGH RISK - RESTRUCTURE OR PASS", color: "#e67e22" };
    return { label: "PASS - DO NOT ADVANCE", color: "#c0392b" };
  };
  ...
  return (
    ...
    >
      <div
        style={{
          background: "linear-gradient(135deg, #0d1120 0%, #141a2e 50%, #0d1120 100%)",
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "20px", marginBottom: "16px" }}>
          ...
          <div>
            <div style={{ fontSize: "11px", letterSpacing: "4px", color: "#c9a84c", textTransform: "uppercase", marginBottom: "4px" }}>
              SoC13 Acquisition Intelligence
            </div>
            <div style={{ fontSize: "24px", fontWeight: "bold", letterSpacing: "1px", color: "#f0e8d8" }}>
        ...
        <div style={{ display: "flex", gap: "24px", flexWrap: "wrap" }}>
          <div style={{ flex: 1, minWidth: "200px" }}>
            <label style={{ fontSize: "10px", letterSpacing: "3px", color: "#c9a84c", display: "block", marginBottom: "6px", textTransform: "uppercase" }}>
              Deal / Target Name
            </label>
            <input
              value={dealName}
      ...
      {totalScored > 0 && (
        ...
        >
          <div>
            <div style={{ fontSize: "10px", letterSpacing: "3px", color: "#8a7a5a", textTransform: "uppercase", marginBottom: "4px" }}>
              Criteria Scored
            </div>
            <div style={{ fontSize: "22px", color: "#c9a84c", fontWeight: "bold" }}>
              {totalScored} <span style={{ fontSize: "14px", color: "#6a6050" }}>/ 13</span>
            </div>
          </div>
          <div>
            <div style={{ fontSize: "10px", letterSpacing: "3px", color: "#8a7a5a", textTransform: "uppercase", marginBottom: "4px" }}>
            ...
            <div style={{ fontSize: "22px", color: "#f0e8d8", fontWeight: "bold" }}>
              {avgScore} <span style={{ fontSize: "14px", color: "#6a6050" }}>/ 5.00</span>
            </div>
          </div>
          {verdict && (
            <div style={{ background: `${verdict.color}18`, border: `1px solid ${verdict.color}55`, padding: "8px 20px" }}>
              <div style={{ fontSize: "10px", letterSpacing: "3px", color: "#8a7a5a", textTransform: "uppercase", marginBottom: "2px" }}>
                Preliminary Verdict
              </div>
              <div style={{ fontSize: "13px", color: verdict.color, fontWeight: "bold", letterSpacing: "1px" }}>{verdict.label}</div>
            </div>
          )}
        </div>
      )}
        ...
        {CRITERIA.map((c) => {
          ...
          return (
            <div
              ...
              style={{
                marginBottom: "10px",
                border: `1px solid ${score ? SCORE_COLORS[score] + "44" : "rgba(201,168,76,0.15)"}`,
                background: score ? SCORE_BG[score] : "#0d1018",
              ...
              >
                <div
                  style={{
                    ...
                    height: "28px",
                    flexShrink: 0,
                    background: score ? SCORE_COLORS[score] : "#1e2030",
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                    fontSize: "11px",
                    fontWeight: "bold",
                    color: score ? "#000" : "#c9a84c",
                ...
                <div style={{ flex: 1 }}>
                  <span style={{ fontSize: "10px", letterSpacing: "3px", color: "#8a7060", marginRight: "10px", fontFamily: "monospace" }}>{c.code}</span>
                  <span style={{ fontSize: "14px", color: "#e8dfc8", fontWeight: "bold" }}>{c.label}</span>
                  {!isOpen && <span style={{ fontSize: "12px", color: "#5a5040", marginLeft: "12px" }}>{c.description.substring(0, 60)}...</span>}
                </div>
                  ...
                  {[1, 2, 3, 4, 5].map((v) => (
                    <div
                      ...
                      style={{
                        ...
                        border: `1px solid ${score === v ? SCORE_COLORS[v] : "rgba(255,255,255,0.1)"}`,
                        display: "flex",
                        alignItems: "center",
                        justifyContent: "center",
                        fontSize: "12px",
                ...
                <div style={{ minWidth: "110px", textAlign: "right", fontSize: "11px", color: score ? SCORE_COLORS[score] : "#3a3a4a", letterSpacing: "1px" }}>
                  {score ? SCORE_LABELS[score].toUpperCase() : "UNSCORED"}
                </div>

                <div style={{ color: "#4a4a5a", fontSize: "12px", marginLeft: "8px" }}>{isOpen ? "▲" : "▼"}</div>
              </div>

              {isOpen && (
                <div style={{ padding: "0 20px 20px 64px", borderTop: "1px solid rgba(201,168,76,0.1)" }}>
                  <p style={{ fontSize: "13px", color: "#9a8a6a", marginTop: "14px", marginBottom: "14px", fontStyle: "italic", lineHeight: "1.6" }}>{c.description}</p>
                  <div style={{ marginBottom: "14px" }}>
                    <div style={{ fontSize: "10px", letterSpacing: "3px", color: "#c9a84c", marginBottom: "10px", textTransform: "uppercase" }}>
                      Diligence Checkpoints
                    </div>
                    {c.questions.map((q, i) => (
                      <div key={i} style={{ display: "flex", gap: "10px", marginBottom: "7px", alignItems: "flex-start" }}>
                        <div style={{ color: "#c9a84c", fontSize: "11px", marginTop: "2px", flexShrink: 0 }}>◆</div>
                        <div style={{ fontSize: "13px", color: "#b0a080", lineHeight: "1.5" }}>{q}</div>
                      </div>
                    ))}
                  </div>
                  <div>
                    <div style={{ fontSize: "10px", letterSpacing: "3px", color: "#c9a84c", marginBottom: "8px", textTransform: "uppercase" }}>
                      Notes / Findings
                    </div>
                    <textarea
                      value={note}
      ...
      <div style={{ padding: "0 40px 20px" }}>
        <div style={{ background: "#0d1018", border: "1px solid rgba(201,168,76,0.15)", padding: "16px 24px", display: "flex", gap: "24px", flexWrap: "wrap", alignItems: "center" }}>
          <div style={{ fontSize: "10px", letterSpacing: "3px", color: "#c9a84c", textTransform: "uppercase" }}>Score Scale</div>
          {[1, 2, 3, 4, 5].map((v) => (
      ...
      {totalScored === 13 && verdict && avgScore && (
        <div style={{ padding: "0 40px 40px" }}>
          <div style={{ background: `${verdict.color}12`, border: `2px solid ${verdict.color}`, padding: "24px 32px" }}>
            ...
            <div style={{ fontSize: "28px", color: verdict.color, fontWeight: "bold", letterSpacing: "2px", marginBottom: "12px" }}>{verdict.label}</div>
            <div style={{ fontSize: "13px", color: "#9a8a6a", lineHeight: "1.7" }}>
              Composite score: <strong style={{ color: "#f0e8d8" }}>{avgScore} / 5.00</strong> across all 13 criteria.
      ...
      <div style={{ padding: "0 40px 40px", fontSize: "10px", color: "#3a3040", letterSpacing: "2px", textTransform: "uppercase" }}>
